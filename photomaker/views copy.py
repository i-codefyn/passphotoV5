...
from PassPhoto import settings
from django.http import HttpResponse
        ...
        if request.method == "POST" or None:
            ...
            for i in range(len(object_ids)):
                ...
                for pk in object_ids:
                    ...
                    image = Image.open(File(obj.file))
                        #bg remove.
                    im = remove(image)
            ...
            if len(object_ids)  == 3:
                ...
                pass_photo  = get_multi_pass_photo(images,n_rows=3)
                buffer = io.BytesIO()
                pass_photo.save(buffer, format = "JPEG")
                img_str = base64.b64encode(buffer.getvalue())
                return HttpResponse(img_str)
            ...
            if len(object_ids)  == 4:
                ...
                pass_photo  = get_multi_pass_photo(images,n_rows=4)
                buffer = io.BytesIO()
                pass_photo.save(buffer, format = "JPEG")
                img_str = base64.b64encode(buffer.getvalue())
                return HttpResponse(img_str)
            ...
            if len(object_ids)  == 5:
                ...
                for pk in object_ids:
                    ...
                    images.append(resized_image)
                    images.append(resized_image)  
                pass_photo  = get_multi_pass_photo(images,n_rows=5)
                buffer = io.BytesIO()
                pass_photo.save(buffer, format = "JPEG")
                img_str = base64.b64encode(buffer.getvalue())
                return HttpResponse(img_str)
            ...
            if len(object_ids)  == 6:
                ...
                for pk in object_ids:
                    ...
                    images.append(resized_image)
                    images.append(resized_image)  
                pass_photo  = get_multi_pass_photo(images,n_rows=6)
                buffer = io.BytesIO()
                pass_photo.save(buffer, format = "JPEG")
                img_str = base64.b64encode(buffer.getvalue())
                return HttpResponse(img_str)
            ...
            if len(object_ids)  == 7:
                ...
                for pk in object_ids:
                    ...
                    images.append(resized_image)
                    images.append(resized_image)  
                pass_photo  = get_multi_pass_photo(images,n_rows=7)
                buffer = io.BytesIO()
                pass_photo.save(buffer, format = "JPEG")
                img_str = base64.b64encode(buffer.getvalue())
                return HttpResponse(img_str)
...
# class PhotosOutputList(LoginRequiredMixin, ListView):
#     """DETAIL VIEWS"""
...
#     #     return self.model.objects.all().filter(created_by=self.request.user)

#     def get_context_data(self, **kwargs):
#         context = super().get_context_data(**kwargs)
#         context["photos"] = self.model.objects.filter(
#             created_by=self.request.user
#         ).order_by("-id")[:10][::-1]
#         return context
...
def pass_photo_single(request, pk):
    ...
    new_img = Image.open(File(photo.file)).convert("RGB")
    #bg remove.
    im = remove(new_img)
    im = im.convert("RGBA")   # it had mode P after DL it from OP
    if im.mode in ('RGBA', 'LA'):
...
def pass_photo_full_4by6(request, pk):
    ...
    # Get the image data from the database.
    new_img = Image.open(File(photo.file))
